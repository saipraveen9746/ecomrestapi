...
from .serializers import CategorySerializer, ProductSerializer,CartSerializer,ProductDetailviewSerializer
...
def add_to_cart(request, product_id):
    if request.method == 'POST':
        try:
            ...
            cart_item, created = CartProduct.objects.get_or_create(product=product, user=request.User)
            cart_item.quantity += 1
            cart_item.save()

            # Serialize cart_item to include its data in the response
